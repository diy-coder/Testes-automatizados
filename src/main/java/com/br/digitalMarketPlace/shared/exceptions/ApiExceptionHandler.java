...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleValidationException(ValidationException ex, WebRequest request) {
        var appException = new AppException();
        appException.setStatus(ex.getStatus().value());
        appException.setTitle(ex.getMessage());
        appException.setDateTime(LocalDateTime.now());

        return handleExceptionInternal(ex, appException, new HttpHeaders(), ex.getStatus(), request);
    }
